...
const projectRoot = "D:/dev/_Projects/RESTRequesting";
...
function contentChange(changeInfo: {
	...
	return new Promise((resolve) => {
		let report = `\n==========\nContent Change: pattern='${changeInfo.filePattern}', ${changeInfo.edits.length} edits`;
		glob(changeInfo.filePattern) .then((files) => {
			report += `\nglob("${changeInfo.filePattern}")`;
			report += `  glob() => count: ${files.length}`;
					...
					//		report (header);
							if (changeCount > 0) {
								fs.writeFile(files[i], content, "utf8", (err) => {
									...
									if (err)
										report += `  ERROR: writing file '${files[i]}'\n    ${err.message}\n`;
									else
										report += `  file '${files[i]}' successfully modified`;
									resolve(report);
								});
							} else
