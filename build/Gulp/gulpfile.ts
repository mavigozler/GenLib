		...
		if (err)
			...
			const defaultAnswers = {
				...
				systemDetail = (selectedConfig: SelectedConfig) => {
					...
					`\n  Selected Gulp config: task = '${selectedConfig.task}', stage = '${selectedConfig.stage}'\n` +
					"\n  ===========================";
				};
			if (process.env) {
	...
	if (!gulpfileConfig.preclean)
		...
		return new Promise<void>((resolve, reject) => {
			...
			deleteAsync(
				...
				gulpfileConfig.preclean.options
						...
						for (const gulpconfigSection in gulpfileConfig?.html)
							if (gulpconfigSection == sectiondef.secName) {
									...
									for (const subsectionProp in gulpfileConfig?.html.scripts) {
										...
										if ((subsectionProp == "special") != null && subsection.length > 0)
											...
											if (subsection.tsconfigPathMaps)
												for (const pathMap of
														subsection.tsconfigPathMaps as tsconfigPathMaps[])
													if (pathMap.from) {
														const fromPattern = pathMap.from.match(/%%([^%]+)%%/);
														if (fromPattern) {
															const basePath = `${GulpDict.get("projectRoot")}/${fromPattern[1]}`;
															const globs = globSync(`${basePath}/**/*.js`);
															const excepts = pathMap.except;
															for (const foundItem of globs) {



																const toTarget = GulpDict.resolveString(
																			`${pathMap.to}/${pathMod.basename(foundItem)}`);
																if (excepts &&
																		excepts.findIndex(elem => foundItem.search(elem) >= 0) >= 0)
																	continue;
								...
								if (gulpfileItems.length > 0) {
									content = content.replace(sectiondef.marker,	buildSection(sectiondef, gulpfileItems));
									gulpfileItems = [];
								}
							}
						}
...
function copyOperations(): Promise<void> {
	...
	return new Promise<void>((resolve, reject) => {
		...
		for (const copyAction of gulpfileConfig.copy) {
			if (("bypass" in copyAction) == true)
				continue;
			const assertCopyAction: {from: string[]; to: string} = copyAction;
					...
					if (resolvedPattern.search(/TSCONFIGFILES|TSCONFIGINCLUDE/) > 0)
						...
						.then((filesSet: [string, string][]) => {
							...
							.then(() => {
								...
								reject(glog(`ERROR copyOperations(): verifyPath()\n` +
								`  errmsg: '${err.message}'`));
							});
						}).catch((err) => {
					...
					// all other files not TS or JS
						glob(resolvedPattern).then((matches) => {
							...
							verifyPath(toPath).then(() => {
								...
								reject(glog(`ERROR copyOperations(): verifyPath(${toPath.length} files)` +
								`\n  errmsg: '${err.message}'`));
							});
						}).catch((err) => {
							reject(glog(`ERROR copyOperations(): glob('${GulpDict.resolveString(fromItem)}')` +
							`\n  errmsg: '${err.message}'`));
						});
				}));
			Promise.all(eachGlob).then((responses) => {
				resolve(glog(`copyOperations(): glob Promise.all() responses = ${responses.length}) globs count: ${globsCount}\n`));
			}).catch((err) => {
				reject(glog(`ERROR copyOperations(): copyAction::Promise.all(): called counts = ${globsCount}` +
					`\n  errmsg: '${err.message}'`));
			});
		}
	});
	...
	*/
	function performCopyAction(
		copyItemSet: string[],   // each source path must be specified
		dest: string    // this value should be path to directory
	): Promise<boolean> {
		glog(`copyOperations()::calling performCopyAction(\n  copyItemSet: ${JSON.stringify(copyItemSet)})` +
			`\n  dest: '${dest}'\n )`);
		return new Promise((resolve, reject) => {
			...
			for (const item of copyItemSet)
				eachCopyAction.push(new Promise((resolve, reject) => {
					copyActionCount++;
					const appendedDestBaseName = fwdSlash(`${dest}/${pathMod.basename(item)}`);
					fs.copyFile(item, appendedDestBaseName, (err) => {
						if (err)
							reject(glog(`ERROR performCopyAction(): request fs.copyFile(` +
								`\n   '${item}' ->` +
								`\n            '${appendedDestBaseName}')` +
								`\n  errmsg: '${err.message}'`));
						else {
							glog(`copied: '${item}' -->\n           '${appendedDestBaseName}'`);
							resolve(true);
...
function editOperations(done: GulpCallback) {
	// open file
	const gulpfileConfig = SelectedGulpfileConfig.config!;
	if (!gulpfileConfig.edits) {
		glog("editOperations()::  gulpfile.config.json specifies no 'edits' property");
		return done();
	}
	if (gulpfileConfig.edits.find(elem => elem.bypass)?.bypass == true) {
		glog("editOperations()::  bypass specified");
		return done();
	}
	...
	for (const editInfo of edits) {
		...
		glob(fPath).then((files) => {
				...
				fs.readFile(file, (err, buffer) => {
					if (err) {
						...
						if (Array.isArray(editInfo.fixes) == false) {
							glog("ERROR: editOperations()::the Gulp config json for 'edits.fixes must be an array of objects. Fix the JSON");
							done();
						} else {
							for (const fix of editInfo.fixes) {
								const re = new RegExp(fix.target, fix.flags ? fix.flags : "");
							...
							fs.writeFile(file, content, (err) => {
								if (err)
									glog("ERROR: editOperations()--writeFile()"+
										`${err.message}`);
								else
									glog(`editOperations('${file}') completed with no error`);
								done();
							});
						}
					}
			...
			glog(`ERROR: editOperations()::glob(${fPath})\n  ${err.message}`);
		});
	}
}
